...
export default function AdminGlobalChat() {
    ...
    axios.get("/api/globalChat/all")
      .then((res) => {
        setChats((prev) => {
          ...
          if (activeCustomer && grouped[activeCustomer]) grouped[activeCustomer].unread = 0;
    ...
    const handler = (payload) => {
      ...
      setChats((prev) => {
        ...
        const unreadIncrement =
          payload.sender === "customer" && activeCustomer !== payload.customerId ? 1 : 0;
  ...
  // Scroll to bottom on new messages or when activeCustomer changes
  useEffect(() => {
 ...
 socket.emit("global_admin_message", message);
  ...
  // Open a customer chat and reset unread
  ...
  const totalUnreadCustomers = Object.values(chats).filter(chat => chat.unread > 0).length;
        ...
        <div className="mt-2 w-[440px] max-w-full h-[540px] bg-white rounded-lg shadow-xl flex overflow-hidden border">
          ...
          {!activeCustomer && (
            <div className="w-56 bg-gradient-to-b from-[#fafafa] to-[#f0f0f0] p-3 border-r border-gray-200 flex flex-col overflow-y-auto">
              <h2 className="text-[#d1383a] font-semibold text-center mb-3 text-lg">Active Chats</h2>
            ...
            <div className="flex-1 flex flex-col">
              <div className="bg-[#d1383a] text-white flex items-center justify-between p-3">
                <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveCustomer(null)}>
                  <FaArrowLeft />
                  <span className="font-semibold truncate">{chats[activeCustomer]?.name}</span>
                </div>
              </div>
                ...
                {chats[activeCustomer]?.messages.map((m,i) => {
                  ...
                  return (
                    <div key={i} className={`flex w-full ${isAdmin ? "justify-end" : "justify-start"}`}>
                      <div className={`relative max-w-[75%] px-4 py-2 rounded-2xl shadow-sm text-sm leading-relaxed ${
                        isAdmin ? "bg-[#d1383a] text-white rounded-br-none" : "bg-white text-gray-800 border border-gray-200 rounded-bl-none"
                      }`}>
                        <div>{m.message}</div>
                        <div className={`text-[10px] mt-1 ${isAdmin ? "text-white/70 text-right" : "text-gray-500 text-left"}`}>{time}</div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="flex p-3 border-t border-gray-200 gap-2 bg-white">
                ...
                />
                <button className="bg-[#d1383a] text-white px-4 py-2 rounded-lg hover:bg-[#a31d06] transition" onClick={sendMessage}>
                  Send
                </button>
              </div>
            </div>
